  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h1 className="font-display text-3xl font-900 uppercase">Catálogo</h1>
        <button
          type="button"
...
/** Confirmação genérica, usada pelo painel para ações destrutivas. */
export function ConfirmDialog({
  ...
  return (
    <div
      ...
      onClick={onCancel}
    >
      <div className="w-full max-w-sm bg-paper p-6" onClick={(e) => e.stopPropagation()}>
        <p className="text-sm text-ink">{message}</p>
        <div className="mt-5 flex justify-end gap-3">
          <button type="button" onClick={onCancel} className="btn btn-outline px-4 py-2 text-xs uppercase">
            Cancelar
          </button>
          <button type="button" onClick={onConfirm} className="btn btn-primary px-4 py-2 text-xs uppercase">
            Confirmar
          </button>
        </div>
      </div>
